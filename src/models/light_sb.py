...
class LightSB(nn.Module):
    def __init__(self, dim=2, n_potentials=5, epsilon=1, is_diagonal=True,
                 sampling_batch_size=1, S_diagonal_init=0.01, r_scale=1):
        super().__init__()
    ...
    def get_drift(self, x, t):
        ...
        # print(f'Autograd outputs: ', epsilon*torch.autograd.grad(lse, x, grad_outputs=torch.ones_like(lse, device=lse.device), create_graph=True)[0].shape)
        drift = (-x/(1-t[:, None]) + epsilon*torch.autograd.grad(lse, x, grad_outputs=torch.ones_like(lse, device=lse.device), create_graph=True)[0]) # + log_alpha.sum()
    ...
    def get_log_potential(self, x):
        ...
        if self.is_diagonal:
            ...
            # print('self.log_alpha.device, S.device, x.device', self.log_alpha.device, S.device, x.device)
            potential = gmm.log_prob(x) + torch.logsumexp(self.log_alpha, dim=-1)
            # print('potential', potential.device)
        else:
            mix = Categorical(logits=self.log_alpha)
